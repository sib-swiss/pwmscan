...
read_pwm(FILE *input, char *iFile)
{
  ...
  int ret;
  while ( l < LMAX && (ret=fscanf(f," %d  %d  %d  %d", &pwm[0][l], &pwm[1][l], &pwm[2][l], &pwm[3][l])) != EOF ) {
    if (ret == 0) {
    ...
    if (l == p_len - 1) {
      ...
      for ( int i = 0; i < NUCL; i++) {
        pwm[i] = realloc(pwm[i], p_len*2*sizeof(int));  /* Rellocate columns */
        if (pwm[i] == NULL) {
...
process_pwm()
{
  ...
  /* Rescale PWM to set min=0 for each pwm position/column */
  ...
  if (options.debug) {
    ...
    for ( i = 0; i < NUCL; i++) {
      ...
      for (val = pwm[i]; val < pwm[i] + pwmLen; val++) {
  ...
  max = 0;
  /* Treat first culumn (first position)      */
  for (i = 0; i < NUCL; i++) { 
    p[pwm[i][0]] += bg[i];
    // printf ("p[%d] = %f\n", pwm[i][0], p[pwm[i][0]]);
  }
  /* max score of first column/position       */
  max += max_score(0);
  // printf("PWM MAX[0] = %d\n", max);
  /* Loop on sub-sequent columns/positions */
  for (k = 1; k < pwmLen; k++) {
    for (j = 0; j <= max;  j++) {
      if (p[j] != 0) {    /* Only consider PWM scores     */
        for (i = 0; i < NUCL; i++) {
            ...
            q[j + pwm[i][k]] += p[j]*bg[i];
            // printf ("q[%d] = %.2e\n", j + pwm[i][k], q[j+pwm[i][k]]);
        }
      }
    } 
    max +=  max_score(k); /* Update score range adding max score of column k */
    // printf("PWM MAX[%d] = %d\n", k, max);
    for (j = 0; j <= max;  j++) {
...
main(int argc, char *argv[])
{
  ...
  if (optind > argc || options.help) {
    fprintf(stderr,
	    ...
	    "\n\tCompute the cumulative score distribution of a given position weight matrix (<pwm_file>) or PWM.\n"
  ...
  for (i = 0; i < NUCL; i++) {
    pwm[i] = calloc((size_t)pwmLen, sizeof(int));  /* Allocate columns */
    if (pwm[i] == NULL) {
      fprintf(stderr, "Out of memory\n");
      return 1;
    }
  ...
  if (options.debug != 0) {
    ...
    for ( int i = 0; i < NUCL; i++) {
      ...
      fprintf(stderr, "[");
      /* processing the rows of the PWM */
      for (p = pwm[i]; p < pwm[i] + pwmLen; p++) {
        fprintf(stderr, " %d ", (*p));
      }
      fprintf(stderr, "]\n");
    }
    fprintf(stderr, "\n");
    fprintf(stderr, "Weight Matrix: original representation \n\n");
    int j = 0;
    for (j = 0; j < pwmLen; j++) {
      for ( int i = 0; i < NUCL; i++) {
        int val = pwm[i][j];
        fprintf(stderr, " %6d ", val);
      }
      fprintf(stderr, "\n");
    }
    fprintf(stderr, "\n");
